...
foreach ($build as $row) {
  ...
  echo "<![CDATA[";
  echo "<p>Spotted <a href=\"https://bmxfeed.com/video/" . $row->video_id . "/\">" . htmlspecialchars_decode($row->video_title) . "</a>. Uploaded by <a href=\"" . $row->video_source_url . "\">" . htmlspecialchars($row->video_source_username, ENT_QUOTES | ENT_IGNORE, "UTF-8") . "</a> on " . date("F j g:ia", strtotime($row->video_date_uploaded)) . ".</p>\n";
