...
foreach ($build as $row) {
  ...
  echo "<title>BMXFEED: " . htmlspecialchars_decode($row->video_title) . "</title>\n";
  echo "<link>https://bmxfeed.com/video/" . $row->video_id . "/</link>\n";
  echo "<description>Uploaded by " . htmlspecialchars_decode($row->video_source_username) . " on " . date("F j g:ia", strtotime($row->video_date_uploaded)) . ".</description>\n";
  echo "<content:encoded>\n";
  echo "<![CDATA[";
  echo "<p>Spotted <a href=\"https://bmxfeed.com/video/" . $row->video_id . "/\">" . htmlspecialchars_decode($row->video_title) . "</a>. Uploaded by <a href=\"" . $row->video_source_url . "\">" . htmlspecialchars_decode($row->video_source_username) . "</a> on " . date("F j g:ia", strtotime($row->video_date_uploaded)) . ".</p>\n";
